...
if (isset($_SESSION['username'])) {
	...
	if (isset($_POST['title'], $_POST['recip'], $_POST['message'])) {
		...
		if ($_POST['title'] != '' and $_POST['recip'] != '' and $_POST['message'] != '') {
			...
			$dn1 = mysqli_fetch_array(mysqli_query($link, 'select count(id) as recip, id as recipid from usernames where username="'.$recip.'"'));
			if ($dn1['recip'] == 1) {
				...
				if ($dn1['recipid'] != $_SESSION['userid']) {
					...
					$key    = thekey($_SESSION['userid'], $dn1['recipid']);
					$tag    = null;
					...
					if (in_array($cipher, $method)) {
						...
						if (mysqli_query($link, 'insert into private (title, sender, recipient, message, timestamp, tag) values ("'.$title.'", "'.$_SESSION['userid'].'", "'.$dn1['recipid'].'", "'.$ciphertext.'", "'.time().'", "'.$tag.'")')) {
?>
